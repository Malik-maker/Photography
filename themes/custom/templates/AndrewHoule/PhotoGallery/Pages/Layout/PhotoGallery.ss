 ...
 <div class="container pb-4 pt-2 pics">
 <article>
<%-- <h2 class="photogallery__title mb-2 gallery">$Title</h2> --%>
<% if $Content %>$Content<% end_if %>

<% if $PaginatedAlbums.Count() > 1 %>
  <section class="photo-albums">
    <% loop $PaginatedAlbums %>
      <div class="photo-album shadow-sm p-2 bg-white rounded">
        <% if $AlbumThumb %>
        ...
        <div class="photo-album__info">
          <h5 class="photo-album__title"><a href="$Link" class="photo-album__info__link" title="View the $Name gallery" style="color:#FE9F21;">$Name</a> <small class="photo-album__count text-muted">($Photos.Count())</small></h4>
